...
export default function PracticeSession({ tool, onComplete, onQuit }) {
      ...
      >
        ...
        {isBreathing && (
          <div style={{ width: '100%', marginBottom: 20 }}>
            <PixelText size={8} color={C.teal} style={{ display: 'block', marginBottom: 10 }}>
              PRACTICE DURATION
            </PixelText>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
              {durationOptions.map((opt) => (
                <button
                  key={opt.value}
                  onClick={() => setBreathDuration(opt.value)}
                  style={{
                    flex: 1,
                    padding: '10px 8px',
                    borderRadius: 4,
                ...
                >
                  <PixelText size={8} color={breathDuration === opt.value ? C.teal : C.grayLt}>
                    {opt.label}
                  </PixelText>
                </button>
              ))}
        ...
        <button
          onClick={() => onQuit(buildPracticeData(false))}
  ...
  if (phase === 'done') {
    return (
      ...
      >
        ...
        <PixelText size={12} color={C.hpGreen} style={{ display: 'block', marginBottom: 8 }}>
          PRACTICE COMPLETE!
        </PixelText>
        <PixelText size={8} color={C.cream} style={{ display: 'block', marginBottom: 24 }}>
          The Storm grows weaker with each practice.
        </PixelText>
        <PixelBtn
          onClick={() => onComplete(buildPracticeData(true))}
          color={C.gold}
          textColor={C.charcoal}
          style={{ width: '100%' }}
        >
  ...
  if (tool?.id === 'breathing') {
      ...
      return (
        ...
        >
          ...
          <PixelText size={7} color={C.grayLt} style={{ display: 'block', marginBottom: 24 }}>
            {breathLabels[breathPhase]} · {Math.floor(elapsed / 60)}:
            {String(elapsed % 60).padStart(2, '0')} / {Math.floor(breathDuration / 60)}:
            {String(breathDuration % 60).padStart(2, '0')}
          </PixelText>
          <PixelBtn
            onClick={togglePause}
            color={C.gold}
    ...
    return (
      ...
      >
        ...
        <PixelText size={8} color={C.grayLt} style={{ display: 'block', marginBottom: 8 }}>
          {breathLabels[breathPhase]} · {Math.floor(elapsed / 60)}:
          {String(elapsed % 60).padStart(2, '0')} / {Math.floor(breathDuration / 60)}:
          {String(breathDuration % 60).padStart(2, '0')}
        </PixelText>
        <PixelText size={14} color={phaseColor} style={{ display: 'block', marginBottom: 16 }}>
        ...
        <div
          style={{
            ...
            height: 120,
            borderRadius: '50%',
            border: `4px solid ${phaseColor}60`,
            ...
            animation:
              breathPhase === 0
                ? 'breatheIn 4s ease-in-out'
                : breathPhase === 2
                  ? 'breatheOut 6s ease-in-out'
                  : 'none',
          }}
        >
          <PixelText size={20} color={phaseColor}>
            {remaining}
          </PixelText>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  padding: '4px 8px',
                  borderRadius: 4,
                  background: i === breathPhase ? phaseColors[i] + '30' : C.cardBg,
                  border: `2px solid ${bColor}`,
                }}
              >
                <PixelText size={6} color={i === breathPhase ? phaseColors[i] : C.grayLt}>
                  {breathDurations[i]}s
                </PixelText>
              </div>
            );
  ...
  if (tool?.id === 'grounding') {
    const gSteps = [
      {
        num: 5,
        text: 'Look around. Name 5 things you can SEE.',
        hint: 'Notice colors, shapes, light...',
      },
      {
        num: 4,
        text: 'Now notice 4 things you can FEEL.',
        hint: 'Your feet on the floor, the air on your skin...',
      },
      {
        num: 3,
        text: 'Listen for 3 things you can HEAR.',
        hint: 'Traffic, birds, your own breathing...',
      },
    ...
    return (
      ...
      >
        ...
        >
          <PixelText size={20} color={C.teal}>
            {gs.num}
          </PixelText>
        </div>
        <PixelText size={10} color={C.cream} style={{ display: 'block', marginBottom: 8 }}>
          {gs.text}
        </PixelText>
          ...
          {gSteps.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 8,
                borderRadius: '50%',
                background: i <= step ? C.teal : C.mutedBorder,
              }}
            />
          ))}
        </div>
        <PixelText size={8} color={C.grayLt}>
          Continue in {10 - timer}s
        </PixelText>
        <button
          onClick={togglePause}
          style={{
  ...
  if (tool?.id === 'allowing') {
    const aSteps = [
      {
        text: "Close your eyes. Feel the anxiety in your body. Don't fight it. Let it be there.",
        sub: "The Storm is not your enemy. It's just energy passing through.",
      },
      {
        text: "Say to yourself: 'This is just anxiety. It will pass. I don't need to escape.'",
        sub: "You don't need to fix it. Just notice it, like watching clouds.",
      },
      {
        text: "Take a deep breath. The Storm is still here, but you're still here too. You're bigger than it.",
        sub: 'Every time you allow the Storm without fleeing, you weaken the Shadow.',
      },
    ];
          ...
          {aSteps.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 6,
                borderRadius: 3,
                background: i <= step ? C.hpGreen : C.mutedBorder,
              }}
            />
          ))}
        </div>
        <PixelText size={8} color={C.grayLt}>
          Continue in {15 - timer}s
        </PixelText>
        <button
          onClick={togglePause}
          style={{
  ...
  if (tool?.id === 'values') {
    const vSteps = [
      {
        text: 'Think about what matters most to you. What kind of person do you want to be?',
        sub: 'Not what you should do — what you deeply care about.',
      },
      {
        text: 'Picture a moment when you lived that value. How did it feel? What did it look like?',
        sub: "That feeling is your anchor. It's always available.",
      },
      {
        text: 'Carry that feeling with you now. This is your compass — not the Storm, not the Shadow.',
        sub: 'When anxiety hits, come back to this. This is who you are.',
      },
    ];
          ...
          {vSteps.map((_, i) => (
            <div
              ...
              style={{
                ...
                height: 6,
                borderRadius: 3,
                background: i <= step ? C.goalGold : C.mutedBorder,
              }}
            />
          ))}
        </div>
        <PixelText size={8} color={C.grayLt}>
          Continue in {12 - timer}s
        </PixelText>
        <button
          onClick={togglePause}
          style={{
