...
export default function RepeatPhase({
  ...
  return (
    <div style={{ animation: 'fadeIn 0.6s ease-out' }}>
      ...
      <div
        style={{
          background: C.hpGreen + '10',
          border: `2px solid ${C.hpGreen}30`,
          borderRadius: 6,
          padding: C.padLg,
          marginBottom: 16,
          textAlign: 'left',
      ...
      >
        <PixelText
          size={7}
          color={C.hpGreen}
          style={{ display: 'block', marginBottom: 8, letterSpacing: 2 }}
        >
          WHY REPEAT?
        </PixelText>
        <PixelText size={7} color={C.cream} style={{ display: 'block', lineHeight: 1.8 }}>
          ...
          {'\n'}
          Every time you repeat the same exposure, your nervous system learns a little deeper that
          the threat isn't real. The Shadow's whispers get quieter. The Storm gets smaller.{'\n'}
          {'\n'}
          This is the heart of ERP therapy — not just facing fear once, but practicing it until it
          becomes normal. That's how you reclaim your life from the Shadow.
        </PixelText>
      </div>
          ...
          <div style={readOnly ? { opacity: 0.7, pointerEvents: 'none' } : undefined}>
            {repeatOptions.map((opt, i) =>
              readOnly ? (
                ...
                <button
                  key={i}
                  onClick={() => setSelectedRepeat?.(opt.text)}
                  style={{
                    display: 'block',
                    width: '100%',
                    marginBottom: 8,
                    padding: '12px 14px',
                    borderRadius: 4,
                    border: `2px solid ${selectedRepeat === opt.text ? C.hpGreen : C.mutedBorder}`,
          ...
          {!readOnly && onRegenerate && (
            ...
            >
              <PixelText size={7} color={C.plumMd}>
                🎲 I FEEL LUCKY — show me something different
              </PixelText>
            </button>
          )}
        </>
      ...
      >
        {continueLabel ||
          (readOnly ? 'GOT IT — ON TO THE PATH →' : "I'M READY TO REPEAT → MISSION COMPLETE")}
      </PixelBtn>
    </div>
  );
}
