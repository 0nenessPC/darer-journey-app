...
export function useCompletionHandlers({
  ...
  const handleBossVictory = useCallback(async (outcome, details = {}) => {
    const { prepAnswers, suds, exposureWhen, exposureWhere, exposureArmory, exposureScheduledTime, battleMessages, victoryMessages, lootImage, lootText, decideSelectedVals, decideCustom, allowFearful, allowLikelihood, allowSeverity, allowCanHandle, allowFearShowing, allowPhysicalSensations, allowCustomSensation, fearedHappened, fearedSeverity, madeItThrough, engageFreeText, repeatChoice } = details;
    ...
    // Check achievements
    const defeatedCount = newHistory.filter(b => b.outcome === "victory" || b.outcome === "partial").filter((b, i, arr) => arr.findIndex(x => x.bossId === b.bossId) === i).length;
    ...
    const maxSudsDrop = Math.max(...newHistory.map(b => {
    ...
    const diffTiers = new Set(newHistory.map(b => {
    ...
    const repeatBosses = newHistory.filter(b => b.bossId?.startsWith('repeat_') && b.suds?.during);
    const bestRepeatSudsDrop = repeatBosses.length > 0
      ? Math.max(...repeatBosses.map(rb => {
          const original = newHistory.find(ob => ob.bossName === rb.bossName && !ob.bossId?.startsWith('repeat_') && ob.suds?.during);
          return original ? (original.suds.during - rb.suds.during) : 0;
        }).filter(v => v > 0), 0)
      : 0;
    const minRepeatSudsAfter = repeatBosses.length > 0
    ...
    const { unlocked: newAchievements } = checkAchievements(
      {
        defeatedCount, victoryCount,
        streakCount: streakUpdate.streakCount,
        coins: (hero.courageCoins || 0) + coinsEarned,
        maxSudsDrop, lootCount, repeatCount,
        nightBattles, allDiffTiers, customBossCount, zonesVisited,
        bestRepeatSudsDrop, minRepeatSudsAfter, repeatVariations,
      },
      prevAchievements,
    );
    ...
    const previousBattle = newHistory.find(b => b.bossName === activeBoss?.name && b.bossId !== activeBoss?.id);
    const sudsImprovement = isRepeatBattle && previousBattle?.suds?.during
    ...
    if (outcome === "victory") {
      ...
      setHero(h => {
        ...
        if (lootDrop) {
          ...
          if (lootDrop.type === 'xp') {
            ...
            collectibles.push({ id: lootDrop.id, name: lootDrop.name, date: new Date().toISOString() });
      ...
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        await saveProgress(user.id, { screen: "map", hero, quest: newQuest, battle_history: newHistory });
    ...
    const { unlocked: newAchObjects } = newAchievements.length > 0
      ? { unlocked: newAchievements.map(id => ({ id, ...ACHIEVEMENTS.find(a => a.id === id) }).filter(Boolean)) }
      : { unlocked: [] };
    ...
    const weekStats = {
      ...
      maxBossLevel: Math.max(...newHistory.filter(b => {
        const d = new Date(b.date);
        return d >= new Date(Date.now() - 7 * 86400000);
      }).map(b => {
        const boss = quest.bosses?.find(qb => qb.id === b.bossId);
      ...
      activeDays: new Set(newHistory.filter(b => {
        const d = new Date(b.date);
        return d >= new Date(Date.now() - 7 * 86400000);
      }).map(b => new Date(b.date).toDateString())).size,
      bestSudsDrop: Math.max(...newHistory.map(b => {
        const peak = b.suds?.during ?? b.suds?.before ?? 0;
        const after = b.suds?.after ?? 0;
        return peak - after;
      }).filter(v => v > 0), 0),
      uniqueBossTypes: new Set(newHistory.filter(b => {
        const d = new Date(b.date);
        return d >= new Date(Date.now() - 7 * 86400000);
      }).map(b => b.bossId)).size,
    ...
    return {
      ...
      weeklyChallengeRewards: weeklyRewards.coins > 0 || weeklyRewards.xp > 0 ? weeklyRewards : null,
    };
  }, [activeBoss, hero, quest, battleHistory, setQuest, setHero, setActiveBoss, setScreen, setBattleHistory]);

  const handleTutorialComplete = useCallback(async (details = {}) => {
    setScreen("exposureSort");
    const { data: { user } } = await supabase.auth.getUser();
    if (user) {
      await saveProgress(user.id, { screen: "exposureSort", hero, quest, tutorial_complete: true, tutorial_loot: { image: details.lootImage, text: details.lootText } });
