...
const FORECAST_MESSAGES = {
  ...
  active: [
    (ctx) => `This week you faced ${ctx.weeklyBattles} shadow${ctx.weeklyBattles > 1 ? 's' : ''} and ${ctx.weeklyVictories} of them fell. The Shadow's getting weaker. Keep going.`,
    (ctx) => `${ctx.weeklyBattles} battle${ctx.weeklyBattles > 1 ? 's' : ''} this week. Your best SUDS drop was ${ctx.bestSudsDrop} points. The Shadow lied about what you could handle — again.`,
    (ctx) => `You've been brave this week, ${ctx.heroName}. ${ctx.weeklyBattles} exposures, ${ctx.weeklyVictories} victories. The path is clearing.`,
  ],

  // Inactive users (3-6 days)
  drifting: [
    (ctx) => `It's been ${ctx.daysSinceLastBattle} days since your last battle. The Shadow doesn't rest when you do. I'm here when you're ready.`,
    (ctx) => `Hey ${ctx.heroName}. The path's been quiet lately. That's okay. But the Shadow doesn't get weaker on its own. Want to face one today?`,
    (ctx) => `${ctx.daysSinceLastBattle} days. You've faced harder things than coming back. I'll be here when you return.`,
  ],

  // Very inactive users (7+ days)
  absent: [
    (ctx) => `I know coming back is the hardest part. It's been ${ctx.daysSinceLastBattle} days, and the Shadow probably told you it's too late to restart. It's wrong. Every path starts with one step — even this one.`,
    (ctx) => `${ctx.heroName}, I remember when you defeated ${ctx.lastBossName}. That took real courage. Whatever made you step away doesn't matter now. What matters is that the path is still here. I'm still here.`,
    (ctx) => `The Shadow thinks you've given up. Want to prove it wrong? Just one exposure. That's all I'm asking.`,
  ],
};
...
export function generateShadowForecast(hero, battleHistory) {
  ...
  const weeklyVictories = battleHistory.filter(b => new Date(b.date) >= weekAgo && b.outcome === 'victory').length;
  const bestSudsDrop = Math.max(
    ...battleHistory.filter(b => new Date(b.date) >= weekAgo).map(b => (b.suds?.before || 0) - (b.suds?.after || 0)).concat([0])
  );
