...
export function useAIChat(systemPrompt, ctx = "") {
  ...
  const init = useCallback(async (prompt) => {
    ...
    const res = await callAI(systemPrompt, [{ role: "user", text: prompt }], 1000, 45000, abortRef.current.signal);
    if (res === FALLBACK) {
  ...
  const reset = useCallback(() => {
    ...
    setMessages([]); hist.current = []; setTyping(false); setError(null); setErrorType(null);
  }, []);
...
No other text.`,
    ...
    if (jsonMatch) {
      ...
      if (Array.isArray(parsed) && parsed.length >= 2) {
        return parsed.slice(0, 3).map((o, i) => ({
    ...
    return [
      { text: `Do "${currentText}" again, but push yourself a bit further`, icon: "🔁", tag: "normal" },
