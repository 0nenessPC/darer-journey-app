...
export default function WelcomeBackLetter({ letterData, onContinue, hero }) {
  ...
  const { daysSinceLastActive, lastBossName, totalDefeated, bestSudsDrop, streakCount } =
    letterData;
      ...
      {totalDefeated > 0 && (
        ...
        >
          <div style={{ textAlign: 'center' }}>
            <PixelText size={12} color={C.hpGreen}>
              {totalDefeated}
            </PixelText>
            <div>
          ...
          <div style={{ textAlign: 'center' }}>
            <PixelText size={12} color={C.bossRed}>
              {bestSudsDrop}
            </PixelText>
            <div>
              <PixelText size={6} color={C.grayLt}>
...
function generateLetterBody(ctx) {
  const { daysSinceLastActive, lastBossName, totalDefeated, bestSudsDrop, streakCount, heroName } =
    ctx;
