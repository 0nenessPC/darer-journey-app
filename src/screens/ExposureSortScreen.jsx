...
export default function ExposureSortScreen({
  hero,
  shadowText,
  onComplete,
  obState = {},
...
- Shadow assessment: ${shadowText || 'Avoidance of social situations, fear of judgment'}`,
      ...
      const parsed = validateAIResponse(
        res,
        z.array(z.object({ name: z.string(), activity: z.string(), level: z.number().optional() })),
      );
      if (Array.isArray(parsed) && parsed.length > 0) {
        const initial = parsed
          .slice(0, 10)
          .map((e, i) => ({ ...e, id: 'exp' + i, level: e.level || i + 1 }));
        setExposures(initial);
        setAllSuggestions(
          initial.map((e) => ({ name: e.name, activity: e.activity, level: e.level })),
        );
      }
    } catch (e) {
      /* AI generation failed — will show retry */
    }
    setLoading(false);
  };
...
- Shadow assessment: ${shadowText || 'Avoidance of social situations, fear of judgment'}`,
      ...
      const parsed = validateAIResponse(
        res,
        z.object({ name: z.string(), activity: z.string(), level: z.number().optional() }),
      );
      if (parsed && parsed.name && parsed.activity) {
        return { ...parsed, id: 'exp_r_' + Date.now(), level: parsed.level || level };
      }
    } catch (e) {
      logger.warn('Replacement generation failed:', e);
    }
    return null;
  };
  ...
  const handleAccept = () => {
    ...
    setLevelRejectCounts((prev) => {
      const next = { ...prev };
      delete next[card.level];
      return next;
    });
    setTimeout(() => {
      setSwipeDir(null);
    ...
    if (rejectionsSoFar < 2) {
      ...
      if (replacement) {
        ...
        setAllSuggestions((prev) => [
          ...prev,
          { name: replacement.name, activity: replacement.activity, level },
        ]);
  ...
  const onTouchMove = (e) => {
    ...
    touchCurrentRef.current = e.touches[0].clientX;
    setDragX(touchCurrentRef.current - touchStartRef.current);
  };
  const onTouchEnd = () => {
    if (!touchStartRef.current) return;
    const diff = touchCurrentRef.current - touchStartRef.current;
    if (diff > 60) handleAccept();
    else if (diff < -60) handleReject();
    else {
      setDragX(0);
      setDragging(false);
    }
  ...
  if (done) {
    ...
    if (accepted.length === 0 && exposures.length > 0) {
      return (
        ...
        >
          <PixelText size={12} color={C.bossRed} style={{ display: 'block', marginBottom: 4 }}>
            THE FORGE IS COLD
          </PixelText>
          ...
          <PixelBtn
            ...
            style={{ width: '100%', maxWidth: 340, marginTop: 12 }}
          >
            FORGE AGAIN →
          </PixelBtn>
        </div>
    ...
    return (
      ...
      >
          ...
          {finalBosses.map((b, i) => (
            ...
            >
              ...
              >
                <div
                  style={{
                    ...
                    height: 20,
                    borderRadius: '50%',
                    background: levelColor(b.level) + '30',
                    border: `2px solid ${levelColor(b.level)}`,
                    display: 'flex',
                    alignItems: 'center',
                ...
                {i < finalBosses.length - 1 && (
                  <div style={{ width: 2, flex: 1, background: C.mutedBorder, minHeight: 12 }} />
                )}
              </div>
              {/* Boss card */}
              <div
              ...
              >
                ...
                >
                  <PixelText size={7} color={levelColor(b.level)}>
                    {b.name}
                  </PixelText>
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div style={{ marginLeft: 8 }}>
              <PixelText size={7} color={C.goalGold}>
                {hero.values?.[0]?.text || 'Freedom from the Shadow'}
              </PixelText>
            </div>
          </div>
        </div>
        ...
        <PixelBtn
          onClick={() => onComplete(finalBosses)}
          color={C.gold}
          textColor={C.charcoal}
          style={{ width: '100%', maxWidth: 340, marginTop: 12 }}
        >
      ...
      {loading ? (
        <div style={{ textAlign: 'center', animation: 'fadeIn 0.5s ease-out' }}>
          ...
          <PixelText size={10} color={C.goldMd} style={{ display: 'block', marginBottom: 8 }}>
            FORGING YOUR BATTLES
          </PixelText>
          <PixelText size={7} color={C.subtleText} style={{ display: 'block' }}>
            Dara is studying your Shadow profile...
          </PixelText>
        </div>
      ) : exposures.length === 0 ? (
        <div style={{ textAlign: 'center' }}>
          <PixelText size={10} color={C.bossRed} style={{ display: 'block', marginBottom: 12 }}>
            THE FORGE NEEDS MORE FIRE
          </PixelText>
          <DialogBox speaker="DARA">
            <PixelText size={8} color={C.cream} style={{ display: 'block', lineHeight: 1.8 }}>
        ...
        <div style={{ width: '100%', maxWidth: 340, textAlign: 'center' }}>
          <PixelText size={9} color={C.goldMd} style={{ display: 'block', marginBottom: 4 }}>
            FORGE YOUR PATH
          </PixelText>
          ...
          >
            <div
              style={{
                height: '100%',
                width: `${(currentCard / exposures.length) * 100}%`,
                background: C.goldMd,
                borderRadius: 2,
          ...
          {generatingReplacement && (
            ...
            >
              ...
              <PixelText size={8} color={C.goldMd} style={{ display: 'block' }}>
                Dara is finding another option...
              </PixelText>
            </div>
          )}
          ...
          <div
            ...
            style={{
              ...
              transition: swipeDir ? 'all 0.3s' : 'none',
            }}
          >
            ...
            <div
              style={{
                ...
                borderRadius: 3,
                marginBottom: 12,
                background: levelColor(card.level) + '20',
                border: `1px solid ${levelColor(card.level)}40`,
              }}
            >
              <PixelText size={6} color={levelColor(card.level)}>
                LV.{card.level} · {levelLabel(card.level)}
              </PixelText>
            </div>
            ...
            {levelRejectCounts[card.level] > 0 && (
              <div
                style={{
                  ...
                  padding: '2px 8px',
                  borderRadius: 3,
                  background: C.plum + '80',
                  border: `1px solid ${C.goldMd}60`,
                  animation: 'fadeIn 0.3s ease-out',
                }}
              >
                <PixelText size={5} color={C.goldMd}>
                  Alt {levelRejectCounts[card.level]} of 2
                </PixelText>
              </div>
            )}

            <PixelText size={11} color={C.cream} style={{ display: 'block', marginBottom: 10 }}>
              {card.name}
            </PixelText>
            <PixelText size={8} color={C.subtleText} style={{ display: 'block', lineHeight: 1.7 }}>
              {card.activity}
            </PixelText>
          ...
          >
            ...
            <PixelText size={8} color={C.subtleText}>
              {currentCard + 1} / {exposures.length}
            </PixelText>
          ...
          <div
            style={{
              ...
              justifyContent: 'center',
              gap: 24,
              opacity: generatingReplacement ? 0.4 : 1,
              pointerEvents: generatingReplacement ? 'none' : 'auto',
            }}
          >
            <button
          ...
          {accepted.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <PixelText size={6} color={C.hpGreen}>
                {accepted.length} battle{accepted.length !== 1 ? 's' : ''} on your path
              </PixelText>
            </div>
          )}
        </div>
