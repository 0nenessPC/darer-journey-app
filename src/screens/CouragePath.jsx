...
function useCourageStats(battleHistory, quest) {
  ...
  // Most repeated boss
  const mostRepeated = repeatedBosses.length > 0
  ...
  return {
    totalBattles, victories, partials, retreats,
    sudsPoints, avgBefore, avgAfter, avgDrop,
    biggestDrop, highestSUDS, lowestAfter,
    repeatedBosses, totalRepeats, mostRepeated,
    courageLevel, totalXP, xpInCurrentLevel,
    defeatedCount, streak, battleDates,
  };
}
...
function SudsChart({ sudsPoints, width = 340, height = 140 }) {
  ...
  const dropFill = points.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.beforeY}`).join(' ')
    + ' ' + [...points].reverse().map(p => `L${p.x},${p.afterY}`).join(' ') + ' Z';
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      ...
      {yLabels.map(v => {
        ...
        return (
          <g key={v}>
            <line x1={padL} y1={y} x2={width - padR} y2={y} stroke={C.mutedBorder} strokeWidth="0.5" strokeDasharray="2,3" />
            <text x={padL - 4} y={y + 3} textAnchor="end" fill={C.grayLt} fontSize="7" fontFamily={PIXEL_FONT}>{v}</text>
          </g>
        );
      })}
      ...
      {/* X-axis: battle count */}
      {points.map((p, i) => (
        <text key={i} x={p.x} y={height - 2} textAnchor="middle" fill={C.grayLt} fontSize="6" fontFamily={PIXEL_FONT}>
          #{i + 1}
        </text>
      ))}
      ...
      <text x={padL + 10} y={padT - 3} fill={C.bossRed} fontSize="6" fontFamily={PIXEL_FONT}>BEFORE</text>
      <circle cx={padL + 54} cy={padT - 6} r="3" fill={C.hpGreen} />
      <text x={padL + 60} y={padT - 3} fill={C.hpGreen} fontSize="6" fontFamily={PIXEL_FONT}>AFTER</text>
    </svg>
  );
}
...
function StatCard({ icon, label, value, subtext, color }) {
  return (
    <div style={{
      ...
      <PixelText size={12} color={color} style={{ display: 'block' }}>{value}</PixelText>
      <PixelText size={6} color={C.grayLt} style={{ display: 'block', marginTop: 2 }}>{label}</PixelText>
      {subtext && (
        <PixelText size={6} color={color} style={{ display: 'block', marginTop: 4, opacity: 0.7 }}>{subtext}</PixelText>
      )}
    </div>
  );
}
...
function PersonalBest({ icon, label, value, color }) {
  return (
    <div style={{
      ...
      <div>
        <PixelText size={8} color={color} style={{ display: 'block' }}>{value}</PixelText>
        <PixelText size={6} color={C.grayLt}>{label}</PixelText>
      </div>
    </div>
  );
}
...
export default function CouragePath({ hero, quest, battleHistory, onBack, setScreen }) {
  ...
  const stats = useCourageStats(battleHistory, quest);
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.mapBg, padding: '16px 16px 100px' }}>
      ...
      <div style={{
        ...
        <PixelText size={10} color={C.goalGold}>🔥 COURAGE PATH</PixelText>
      </div>
      ...
      <div style={{
        ...
        {/* XP progress bar */}
        <div style={{
          height: 6, background: C.cardBg, borderRadius: 3, marginTop: 8,
          border: `1px solid ${C.goalGold}20`, overflow: 'hidden',
        }}>
          <div style={{
            height: '100%', width: `${(stats.xpInCurrentLevel / 300) * 100}%`,
            background: C.goalGold, borderRadius: 3, transition: 'width 0.3s',
          }} />
        </div>
      </div>
      ...
      {(() => {
        ...
        const weekStats = {
          ...
          maxBossLevel: Math.max(...battleHistory.filter(b => {
            const battleDate = new Date(b.date);
            const weekAgo = new Date(Date.now() - 7 * 86400000);
            return battleDate >= weekAgo;
          }).map(b => {
          ...
          activeDays: new Set(battleHistory.filter(b => {
            const battleDate = new Date(b.date);
            const weekAgo = new Date(Date.now() - 7 * 86400000);
            return battleDate >= weekAgo;
          }).map(b => b.date.slice(0, 10))).size,
          bestSudsDrop: Math.max(...battleHistory.filter(b => {
            const battleDate = new Date(b.date);
            const weekAgo = new Date(Date.now() - 7 * 86400000);
            return battleDate >= weekAgo;
          }).map(b => (b.suds?.before || 0) - (b.suds?.after || 0)).concat([0])),
          uniqueBossTypes: new Set(battleHistory.filter(b => {
            const battleDate = new Date(b.date);
            const weekAgo = new Date(Date.now() - 7 * 86400000);
            return battleDate >= weekAgo;
          }).map(b => b.bossName)).size,
          ...
          minSudsAfter: Math.min(...battleHistory.filter(b => {
            const battleDate = new Date(b.date);
            const weekAgo = new Date(Date.now() - 7 * 86400000);
            return battleDate >= weekAgo && b.suds?.after !== undefined;
          }).map(b => b.suds.after).concat([100])),
          toolsUsed: new Set(battleHistory.filter(b => {
            const battleDate = new Date(b.date);
            const weekAgo = new Date(Date.now() - 7 * 86400000);
            return battleDate >= weekAgo && b.exposureArmory;
          }).map(b => b.exposureArmory)).size,
        };
        ...
        return (
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <PixelText size={9} color={C.goalGold}>⚡ WEEKLY CHALLENGES</PixelText>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {weeklyResults.map(ch => (
                <div key={ch.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <PixelText size={7} color={ch.completed ? C.hpGreen : C.cream} style={{ display: 'block' }}>
                      {ch.name}
                    </PixelText>
                    <PixelText size={6} color={C.grayLt}>{ch.desc}</PixelText>
                  </div>
                  {ch.completed ? (
                    <PixelText size={8} color={C.hpGreen}>✓</PixelText>
                  ) : (
      ...
      {!hasData ? (
        ...
        <>
            ...
            <div style={{
              ...
              {stats.avgBefore !== null && stats.avgAfter !== null && (
                <div style={{
                  ...
                  marginTop: 8, paddingTop: 8, borderTop: `1px solid ${C.mutedBorder}`,
                }}>
          ...
          <div style={{
            ...
            <StatCard icon="🔄" label="Repeats" value={stats.totalRepeats} color={C.teal}
              subtext={stats.mostRepeated ? `"${stats.mostRepeated.name}" ${stats.mostRepeated.count}x` : ''} />
            <StatCard icon="🔥" label="Day streak" value={stats.streak} color={C.goldMd}
              ...
              {stats.totalBattles > 0 && (
                <>
                  <div style={{
                    ...
                    background: C.hpGreen, borderRadius: 2,
                  }} />
                  <div style={{
                  ...
                  <div style={{
                    ...
                    background: C.bossRed, borderRadius: 2,
                  }} />
                </>
              )}
            </div>
            <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
              <PixelText size={6} color={C.hpGreen}>Victory: {stats.victories}</PixelText>
              <PixelText size={6} color={C.amber}>Partial: {stats.partials}</PixelText>
              <PixelText size={6} color={C.bossRed}>Retreat: {stats.retreats}</PixelText>
            </div>
          </div>
                ...
                return (
                  <div key={r.name} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <PixelText size={8} color={C.teal}>{r.name}</PixelText>
              ...
              <div style={{
                ...
                {collectibles.map((c, i) => (
                  <div key={i} style={{
                    ...
                    <PixelText size={5} color={C.goldMd}>#{i + 1}</PixelText>
                  </div>
                ))}
              </div>
            </div>
